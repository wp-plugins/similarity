...
Description: Returns links to similar posts. Similarity is determined by the way posts are tagged or by their categories. Compatible with Wordpress 2.3 and 2.5.
Version: 1.0
Author: David Miller
Author URI: http://www.dmidcorp.com/
*/
...
function get_list($type = 'tag') {
	...
	$strength_list = array();
	$potential = 0;
	$query = "select r.term_taxonomy_id as ttid, t.count as rarity from $wpdb->term_relationships r, $wpdb->term_taxonomy t where r.object_id = '$post->ID' and r.term_taxonomy_id in (select term_taxonomy_id from $wpdb->term_taxonomy where taxonomy = '";
	switch ($type) {
	case "cat":
		$query .= "category";
		break;
	...
	$query .= "' and count > 1) and t.term_taxonomy_id = r.term_taxonomy_id order by t.count";
	$results = $wpdb->get_results($query);
	if (count($results)) {
		foreach ($results as $result) {
			$potential += (1 / $result->rarity);
			$query = "select object_id as ID from $wpdb->term_relationships where term_taxonomy_id = $result->ttid and object_id != $post->ID";
